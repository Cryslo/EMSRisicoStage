...
public class DirectoryBox
{
	...
	{	
		...
		if(boxNumber != 0) {
			...
			backgroundImage = Resources.Load<Texture2D>("Backgrounds/FolderBackgrounds/WhiteBackground");
		}
		...
		folderBox.transform.position = mainCamera.ScreenToWorldPoint(new Vector3((mainCamera.pixelWidth + (width / 2)) - ((width * columns) - width * x ), (mainCamera.pixelHeight - (height / 2)) - (height * y), 1));
